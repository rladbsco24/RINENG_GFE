...
def prepare(root=ROOT, *, output_root=None):
    ...
    ctx = pipeline.prepare_pipeline(run_mode="quick", recompute=False,
                                    output_root=(root / "smoke_outputs" if output_root is None
    ...
    (root / "resolved_execution_settings.json" if output_root is None else
     ctx.output_root / "resolved_execution_settings.json").write_text(json.dumps({
...
def main_figure(ctx, number):
    ...
    if ctx.memo.get("notebook_mode") == "supersmoke":
        from run_profiles import label_preview_tables
...
def data_exports(root=ROOT):
    ...
    folders = {
        ...
        "F": root / "appendix_outputs/F_optimizer",
    ...
    for section, directory in folders.items():
        for path in sorted(directory.rglob("*")):
            ...
            if path.name.startswith("exportable_data_index"):
                continue
            # Inspect paths relative to the study root.  An enclosing workspace
            # directory may legitimately be named ``tmp`` and must not suppress
            # every export in the study.
            relative_parts = path.relative_to(root).parts
            if any(x in relative_parts for x in ("__pycache__", "runtime", "tools", "qa", "tmp")):
                continue
    ...
    (output / "README.md").write_text(
        ...
        "B contains Twin/Bottle formulation references. C groups three focused sensitivity "
        "figures for objective weights, multitrap regularization and RH pressure weight. "
        "Appendix F reports FE/GFE with compact L-BFGS-B and optimizer alternatives for Conventional on unchanged objectives. "
...
def export_measured_summary(root, output):
    ...
    for section, directory in (("A", root / "appendix_outputs/A_GFE"),
        ...
        if section == "C":
            # Current summary sources only; archived explorations keep their own data.
            sources = sorted((directory / "reviewer_focused").glob("*summary*.md"))
            sources += sorted((directory / "reviewer_focused").glob("*summary*.txt"))
        for path in sources:
            if path.is_file():
...
def figure_paths(root=ROOT):
    """Return exactly the active 22 figures in notebook order."""
    from production_reporting import PAPER_MAIN_STEMS, APPENDIX_IDS, is_active_figure
    root = Path(root).resolve()
    bases = {
        "Main": root / "smoke_outputs/figures",
        "A": root / "appendix_outputs/A_GFE",
        "B": root / "appendix_B_pressure/figures",
        "C": root / "appendix_outputs/C_GFE/reviewer_focused/figures",
        "D": root / "appendix_outputs/D",
        "E": root / "appendix_outputs/E",
        "F": root / "appendix_outputs/F_optimizer",
    }
    ...
    for identifier, stem in PAPER_MAIN_STEMS:
        candidate = bases["Main"] / f"{stem}.pdf"
        if not candidate.is_file() or not candidate.with_suffix(".png").is_file():
            raise RuntimeError(f"Expected current paper Figure {identifier}: {candidate}")
        paths.append(("Main", candidate))
    for identifier in APPENDIX_IDS:
        section = identifier[0]
        base = bases[section]
        candidates = [p for p in base.rglob(f"Figure_{identifier}_*.pdf")
                      if p.with_suffix(".png").is_file() and is_active_figure(p, root)]
        if len(candidates) != 1:
            raise RuntimeError(f"Expected one active Figure {identifier}, found {candidates}")
        paths.append((section, candidates[0]))
    return paths
