...
def _solve(ctx, objective, initial, seed, role):
    payload = dict(contract="main-feg-same-start-v1", role=role,
        ...
        initial_phase_content_id=digest_array(initial), positions=digest_array(ctx.positions_m),
...
def _updated_row(row, run, objective, status):
    ...
    row.update(method="GFE", objective_method="GFE", alpha_per_m=10.,
        ...
        optimizer_message=str(run.message), phase_content_id=digest_array(run.phase_rad),
...
def feg_single_bank(ctx):
    ...
    if ctx.memo.get("study_scale", {}).get("profile") == "supersmoke":
        from . import production_main as production
        base = production._single_plot_bank(ctx)
        table = base["table"].copy()
        table["method"] = table.method.replace({"FE": "GFE"})
        runs = {("GFE" if method == "FE" else method, index): run
                for (method, index), run in base["runs"].items()}
        result = dict(base, table=table, runs=runs,
                      caps={"Conventional": 10000, "GFE": 10000})
...
def feg_xz_domain_bank(ctx):
    ...
    if ctx.memo.get("study_scale", {}).get("profile") == "supersmoke":
        from . import production_main as production
        base = production._domain_plot_bank(ctx)
        summary = base["summary"].copy()
        summary["method"] = summary.method.replace({"FE": "GFE"})
        value = dict(base, summary=summary)
        ctx.memo[key] = value
        return value
...
def _decision_plot_adapter(ctx):
    ...
    for label in runs:
        ...
        sections[label]=p._native_relative_sections(objectives[label],runs[label],directions,
            line_points=ff._plot_points(ctx,"objective_line_points",101),
            plane_points=ff._plot_points(ctx,"objective_plane_points",101 if ctx.memo.get("production_main") else 51))
...
def feg_triple_run(ctx):
    ...
    payload=dict(contract="fig6-fixed-effective-gravity-compensated-triple-fe-v1",problem=problem.fingerprint,
        ...
        initial_phase_content_id=digest_array(initial),fixed_iteration_cap=cap,
...
def _triple_plot_adapter(ctx):
    if ctx.memo.get("study_scale", {}).get("profile") == "supersmoke":
        from . import production_main as production
        return production._representative_triple_bank(ctx, compensated=True, role="controlled")
    ...
    ctx.tables["main_fe_control_triple_command"]=pd.DataFrame([dict(method="FE",
        phase_content_id=digest_array(node["FE_phase"]),iterations=node["FE"].iterations,
