...
def _d1_worker(root, output, campaign_id):
    ...
    with threadpool_limits(limits=1):
        ...
        identity = dict(schema=SCHEMA+"-D1-timing", environment=environment,
                        ...
                        source_phase_content_id=_content_id(source / "continuous_FE_phases.npz"),
                        ...
                        source_backend_content_id=_content_id(source / "code/inputs/hat_geometry_branch_benchmark.py"))
        identity_hash = content_identity(json.dumps(identity, sort_keys=True).encode()).hexdigest()
                ...
                for q, method in pending:
                    ...
                    for repetition in range(D1_REPEATS):
                        ...
                        block.append(dict(case_id=case,levels=q,method=method,repetition=repetition,
                            transition_ms=elapsed,phase_indices_content_id=digest_array(stable_indices),
...
def _d3(root,ctx,output):
    ...
    for task in ("Single","Triple"):
        ...
        assert digest_array(phase)==source_records[task]["phase_content_id"]
        ...
        for q in support.LEVELS:
            for method,label in (("Nearest","nearest"),("Elastic mechanics offset","offset")):
                for index,(target,reference) in enumerate(zip(command["targets"],command["results"])):
                    ...
                    row.update(evidence_status="PRODUCTION",phase_content_id=digest_array(discrete),
                    ...
                    if not reference.equilibrium.numerical_root_found:
                        ...
                        row["additional_equilibrium_shift_m"]=np.nan
                    records.append(row)
    ...
    summary.to_csv(output / "D3_GFE_quantization_summary.csv",index=False)
    ...
    _json(output / "D3_production_provenance.json",dict(schema=SCHEMA,evidence_status="PRODUCTION",
        ...
        production_source_content_id=_content_id(__file__),source_phase_archive_content_id=_content_id(source / "D3_GFE_source_commands.npz"),
...
def _e(root,output):
    ...
    _json(output / "production_replay_provenance.json",dict(schema=SCHEMA,
        ...
        phase_profile_archive_content_id=_content_id(source / "all_candidate_commands_profiles.npz"),
...
def run(root,ctx):
    ...
    if reference["resolved_reference_targets"]:
        d_caption=d_caption.replace("(b) Absolute distance from requested target.",
            ...
            f"({reference['absolute_target_error_m']:.3e} m across {reference['resolved_reference_targets']} resolved targets).")
    else:
