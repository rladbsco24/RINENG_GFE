"""Solve gravity-balanced landmarks on the deposited target chart.
...
each is independently reoptimized with the correct-sign GFE objective,
alpha=10, and a 10,000-iteration cap.  Conventional trajectories are then
placed out of sample in one GFE-only three-dimensional projective-MDS frame.
"""
...
def _continue_one(
    ...
    from .main_feg import feg_single_objective
    objective = feg_single_objective(ctx, target)
    ...
    payload = {
        "contract": "alpha10-feg-two-branch-medoid-continuation-v1",
    ...
    result, cache_status, _ = ctx.cache.get_or_compute(
        "alpha10_feg_branch_medoid_continuation",
        payload,
...
def fixed_feg_branch_pack(ctx: p.PipelineContext) -> dict[str, Any]:
    """Return alpha=10 GFE anchors, projected C webs, and root correlations."""

    scale = ctx.memo.get("study_scale", {})
    supersmoke = scale.get("profile") == "supersmoke"
    chart_side = int(scale.get("branch_chart_side", 3)) if supersmoke else None
    memo_key = "alpha10_feg_branch_pack"
    if supersmoke:
        memo_key += f":supersmoke:{chart_side}"
    if memo_key in ctx.memo:
        return ctx.memo[memo_key]

    old_medoids = ctx.branch_bank.medoid_table.copy().reset_index(drop=True)
    selected_targets: set[tuple[int, int]] | None = None
    if supersmoke:
        if chart_side != 3:
            raise ValueError("The supersmoke branch chart uses a centered 3-by-3 grid")
        root_ix, root_iy = ctx.branch_bank.root_target
        selected_axes = []
        for column, root_index in (("ix", root_ix), ("iy", root_iy)):
            indices = sorted(old_medoids[column].unique().astype(int).tolist())
            center = indices.index(int(root_index))
            if center == 0 or center == len(indices) - 1:
                raise ValueError("The source branch chart has no centered 3-by-3 root neighborhood")
            selected_axes.append(indices[center - 1:center + 2])
        selected_targets = {
            (ix, iy) for ix in selected_axes[0] for iy in selected_axes[1]
        }
        selected_mask = [
            (int(row.ix), int(row.iy)) in selected_targets
            for row in old_medoids.itertuples(index=False)
        ]
        old_medoids = old_medoids.loc[selected_mask].reset_index(drop=True)
        counts = old_medoids.groupby(["ix", "iy"])["component"].nunique()
        if len(counts) != 9 or not counts.eq(len(COMPONENTS)).all():
            raise ValueError("Every supersmoke branch target must contain both source components")
    # source_endpoint_position always indexes the complete FE archive, even
    # when the plot selects only nine targets from that archive.
    ...
    edges = ctx.branch_bank.edge_table.copy()
    if selected_targets is not None:
        edges = edges.loc[
            [
                (int(row.a_ix), int(row.a_iy)) in selected_targets
                and (int(row.b_ix), int(row.b_iy)) in selected_targets
    ...
    plot_trajectories = trajectory_table
    if selected_targets is not None:
        plot_trajectories = trajectory_table.loc[
            [
                (int(row.ix), int(row.iy)) in selected_targets
                for row in trajectory_table.itertuples(index=False)
            ]
        ]
    # Keep the archive row indices: Figure 3 uses selected_trajectory_index
    # to retrieve the original Conventional seed from ctx.artifact.
    selected: list[tuple[int, str]] = []
    assigned = np.empty(len(trajectory_table), dtype=object)
    for (ix, iy), group in plot_trajectories.groupby(["ix", "iy"], sort=True):
    ...
    evolution["reference_method"] = "GFE"
    evolution["similarity_to_matched_feg"] = evolution["similarity_to_matched_fe"]
    evolution["distance_to_matched_feg"] = evolution["distance_to_matched_fe"]
    ...
    correlation["reference_method"] = "GFE"
    correlation["similarity_to_matched_feg"] = correlation["similarity_to_matched_fe"]
    correlation["distance_to_matched_feg"] = correlation["distance_to_matched_fe"]
        ...
        [
            {
                ...
                "frame": "GFE-only projective-distance MDS, fixed before Conventional projection",
                "reference_method": "GFE",
                "force_target_z_n": completed[0][0]["force_target_z_n"],
                "worker_count": workers,
            }
        ]
    )
    ctx.tables["fig2_fixed_single_feg_branch_anchors"] = anchor_table.copy()
    ctx.tables["fig2_fixed_feg_conventional_evolution"] = evolution.copy()
    ctx.tables["fig2_fixed_feg_continuation_protocol"] = protocol
    value = {
        "bank": bank,
        "frame": frame,
        "anchors": anchors,
...
__all__ = ["FIXED_ALPHA_PER_M", "ITERATION_CAP", "fixed_feg_branch_pack"]
