...
class ArtifactWriter:
    ...
    def save_figure(self, fig: mpl.figure.Figure, stem: str, dpi: int = 300) -> list[Path]:
        ...
        for path in paths:
            ...
            self.records.append(
                {"kind": "figure", "path": str(path.relative_to(self.root)), "bytes": path.stat().st_size, "fingerprint": self._fingerprint(path)}
            )
        return paths
    ...
    def record_file(self, path: str | Path, kind: str) -> None:
        path = Path(path)
        self.records.append(
            {"kind": kind, "path": str(path.relative_to(self.root)), "bytes": path.stat().st_size, "fingerprint": self._fingerprint(path)}
        )
    ...
    def write_manifest(self, extra: dict[str, object] | None = None) -> Path:
        ...
        for relative, record in latest.items():
            ...
            record["fingerprint"] = self._fingerprint(current)
