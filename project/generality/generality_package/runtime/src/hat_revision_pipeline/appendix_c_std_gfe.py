...
def _solve(root,ctx,problem,spec,provided=None):
    ...
    payload=dict(schema=SCHEMA,problem=problem.fingerprint,seed=spec['seed'],initial_phase_fingerprint=digest_array(phase0),
        config=config.to_payload(),std_scale_multiplier=spec['std_scale_multiplier'],physics_source_fingerprint=c._hash_sources(),
    ...
    row=dict(spec,task='Triple',method='FE' if spec['ablation']=='FE' else 'GFE',alpha=3000.,alpha_factor=1.,
        phase_fingerprint=digest_array(result.phases),initial_phase_fingerprint=digest_array(phase0),
...
def _reference_correspondence(ctx,jobs):
    ...
    for job in jobs:
        ...
        assert job['row']['reference_phase_fingerprint']==ref['row']['phase_fingerprint']
...
def _write_text(root,ctx,commands,mechanics):
    ...
    protocol=dict(schema=SCHEMA,paired_seeds=SEEDS,commands=len(commands),target_count=len(mechanics),
        ...
        physics_source_fingerprint=c._hash_sources(),dependencies=dependency_snapshot(),evidence_status='SMOKE')
