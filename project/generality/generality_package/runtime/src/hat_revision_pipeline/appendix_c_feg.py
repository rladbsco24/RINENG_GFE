...
def _hash_sources():
    ...
    return {name:fingerprintlib.fingerprint((base/name).read_bytes()).hexdigest()
    ...
    for folder in folders:
        for path in sorted(folder.glob('*.pkl')):
            ...
            key=(q.get('problem'),_canonical_config(config),q.get('seed'),
                 q.get('initial_phase_fingerprint',q.get('initial_phase',q.get('initial'))))
...
def _solve_one(root,ctx,problem,source_index,spec,provided=None):
    ...
    row=dict(spec,**native,phase_fingerprint=digest_array(phase),initial_phase_fingerprint=digest_array(initial),
    ...
    for task in ('Single','Triple'):
        ...
        for seed in sorted({j['row']['seed'] for j in selected}):
            ...
            for job in paired:
                ...
                job['row'].update(reference_run_id=reference['row']['run_id'],reference_phase_fingerprint=reference['row']['phase_fingerprint'],
...
def _validate(root,ctx,jobs,workers):
    ...
    def one(request):
        job,index,target=request;row=job['row'];content_id=row['phase_fingerprint'];key=(content_id,tuple(target))
        ...
        out.update(run_id=row['run_id'],task=row['task'],seed=row['seed'],ablation=row['ablation'],alpha=row['alpha'],
                   ...
                   phase_fingerprint=content_id,validation_source=source,validation_cache_status=status,evidence_status=getattr(ctx,'evidence_status','SMOKE'))
...
def _write_text(output,commands,mechanics):
    ...
    for row in reference.itertuples():
        refs.append(dict(task=row.task,seed=row.seed,run_id=row.run_id,phase_fingerprint=row.phase_fingerprint,
    ...
    (output/'C_protocol.json').write_text(json.dumps(dict(schema=SCHEMA,alpha_factors=ALPHA_FACTORS,
        ...
        physics_source_fingerprint=_hash_sources(),dependencies=dependency_snapshot(),evidence_status='SMOKE'),indent=2))
