...
def _import_or_solve(config,legacy,output,module,cache_only):
    ...
    for directory in (legacy/'data/sensitivity_cache',legacy/'data/cache'):
        for source in sorted(directory.glob('*.json')):
            ...
            if fingerprintlib.fingerprint(arrays['terminal_phase_rad'].tobytes()).hexdigest()!=row['phase_fingerprint']:raise ValueError('Invalid source phase')
...
def _pressure(config,arrays,output,cache_only):
    ...
    stamp=dict(schema=SCHEMA,phase_fingerprint=digest_array(phase),pressure_points=181,half_width_m=float(axis[-1]),
        target_m=config['target_m'],frequency_hz=config['frequency_hz'],source_hash=config['source_fingerprint']['gorkov_core.py'])
    key=fingerprintlib.fingerprint(json.dumps(stamp,sort_keys=True).encode()).hexdigest()
    ...
    for identity,subset in frame.groupby(groups,dropna=False):
        ...
        for metric in metrics:
            ...
            out.append(dict(zip(groups,identity))|dict(metric=metric,n_valid=len(vals),n_total=len(subset),median=median,q25=low,q75=high,
                ci95_low=ci[0],ci95_high=ci[1],bootstrap_seed=20260906,bootstrap_resamples=10000,unit='Paired initialization; no pixel resampling'))
...
def run(root,*,ctx=None,cache_only=False,workers=3):
    ...
    for use in uses.itertuples():
        ...
        if use.section in ('C4','C5'):
            ...
            row.update(reference_seed=int(use.seed),reference_method='FE',reference_eta=.01,fixed_fe_reference_cache_key=ref,
                fixed_fe_reference_phase_fingerprint=rows[ref]['phase_fingerprint'],phase_command_similarity_to_fixed_fe=similarity,
        ...
        if use.section=='C4':c4.append(row)
            ...
            row.update(matched_FE=use.beta_factor==0,shared_sweep_peak_pa=peak,target_pressure_over_shared_sweep_peak=float(abs(field[90,90])/peak),
                xz_amplitude_cosine_to_nominal_conventional=s.cosine(field,fields[ref]['XZ']),conventional_reference_phase_fingerprint=rows[ref]['phase_fingerprint'])
            c5.append(row)
        elif use.section=='B1':
            ...
            row.update(reference_method=use.method,xz_amplitude_cosine_similarity=s.cosine(field,fields[ref]['XZ']),xz_amplitude_pearson_correlation=float(np.corrcoef(a,b)[0,1]),
                ...
                command_cosine_to_conventional=s.command_similarity(arrays[key]['terminal_phase_rad'],arrays[ref]['terminal_phase_rad']),conventional_phase_fingerprint=rows[ref]['phase_fingerprint'])
            b2.append(row)
    ...
    protocol=dict(schema=SCHEMA,status='PRODUCTION',paired_seeds=B10,unique_commands=250,C45_unique_commands=200,B1_commands=20,B2_commands=60,
        ...
        objective_signs={'standard':'Kp|p|^2-Kv|v|^2','alternative':'Kp|p|^2+Kv|v|^2'},solver='BFGS',maxiter=10000,gtol=1e-8,
