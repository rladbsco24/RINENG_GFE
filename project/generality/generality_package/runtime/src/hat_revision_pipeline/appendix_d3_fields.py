...
def _pressure_data(output, ctx, commands, *, allow_unresolved=False):
        ...
        for method, label in (("Nearest", "nearest"), ("Elastic mechanics offset", "offset")):
            for index, reference in enumerate(command["results"]):
                ...
                root_rows.append(dict(task=task, levels=levels, target_index=index + 1,
                                      ...
                                      method=method, phase_fingerprint=digest_array(phase),
                                      continuous_phase_fingerprint=digest_array(command["phase"]),
    ...
    provenance = dict(schema="GFE-D3-spatial-pressure-v1", levels=levels,
                      ...
                      command_fingerprint={key: digest_array(value) for key, value in sources.items()},
    ...
    numerical_identity = {key: value for key, value in provenance.items() if key != "implementation_fingerprint"}
    cache_hit = False
...
def plot(output, ctx, commands, *, evidence_status="SMOKE", selection_scope=None, allow_unresolved=False):
    ...
    with plt.rc_context(style):
        ...
        for row, command in enumerate(commands):
            ...
            field_ax.text(.03, .97, f"Isobars: {contour_levels[0] / 1e3:.2f}–{contour_levels[-1] / 1e3:.2f} kPa",
                          transform=field_ax.transAxes, va="top", fontsize=10.5,
            ...
            for method, marker in (("Nearest", "s"), ("Elastic mechanics offset", "o")):
                ...
                for record in part.to_dict("records"):
                    x, z = record["added_shift_x_um"], record["added_shift_z_um"]
            ...
            if task == "Single":
                shift_ax.set(xlim=(-90, 90), ylim=(-25, 155), xticks=(-80, 0, 80), yticks=(0, 50, 100, 150))
            else:
                shift_ax.set(xlim=(-100, 100), ylim=(-140, 60), xticks=(-80, 0, 80), yticks=(-120, -80, -40, 0, 40))
            if allow_unresolved:
                finite = task_roots[["added_shift_x_um", "added_shift_z_um"]].dropna().to_numpy()
                if len(finite):
                    finite = np.vstack([finite, np.zeros(2)])
                    lower, upper = finite.min(0), finite.max(0)
                    span = max(float(np.max(upper-lower))*1.30, 180.)
            ...
            shift_ax.set(xlabel=r"$x_{\rm discrete}-x_{\rm continuous}$ (µm)",
                         ylabel=r"$z_{\rm discrete}-z_{\rm continuous}$ (µm)")
            ...
            values = [task_roots[task_roots.method.eq(method)].added_shift_3d_um.max()
                      for method in ("Nearest", "Elastic mechanics offset")]
            annotation = (f"{'Worst ' if task == 'Triple' else ''}3D shift: {values[0]:.1f} → {values[1]:.2f} µm"
                          if np.isfinite(values).all() else "Shift unresolved")
            if allow_unresolved and not (task_roots.root_found & task_roots.reference_root_found).all():
                annotation += f"; resolved {int(task_roots.root_found.sum())}/{len(task_roots)}"
            shift_ax.text(.03, .97, annotation,
                          transform=shift_ax.transAxes, va="top", fontsize=11, weight="semibold")
            for ax in (field_ax, shift_ax):
                for tick in ax.get_xticklabels() + ax.get_yticklabels():
                    tick.set_fontweight("semibold")
        handles = [Line2D([], [], color=COLORS["Nearest"], ls="--", marker="s", mfc="white", mew=1.8, lw=1.8,
                          label=f"Nearest rounding (Q={ILLUSTRATION_LEVELS})"),
                   Line2D([], [], color=COLORS["Elastic mechanics offset"], marker="o", mfc="white", mew=1.8, lw=1.8,
                          label="Mechanics-aware offset"),
                   Line2D([], [], color="#252525", marker="+", ms=10, mew=2, ls="none", label="Continuous equilibrium")]
    ...
    caption = (
        ...
        "The annotations report full 3D distances, including the unplotted y component. The common phase offset "
