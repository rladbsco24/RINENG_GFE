...
def run_std(root,ctx,workers=None):
    from . import production_main as main
    root=Path(root).resolve();settings=_settings(root,ctx);plan=design(settings)
    ...
    paths=std.render(root,output_dir=base)
    ...
    _caption(base,'Figure_C2_GFE_multitrap_regularization_ablations','C2. '+common+
        ...
        'The STD term balances local objective losses rather than displacement. The dotted 1e-3 line is the L2 reporting threshold, distinct from BFGS infinity-norm gtol=1e-8. '
...
def _c3_jobs(root,shared,local,seed):
    ...
    for i,alpha in enumerate(warm.ALPHAS):
        ...
        if row.initial_phase_fingerprint!=digest_array(initial):raise ValueError('Main7 cold origin differs from C3')
    ...
    for alpha in warm.ALPHAS:
        ...
        payload=dict(contract=warm.SCHEMA,method=asdict(method),force_target_n=current.force_target_n.tolist(),
            ...
            frequency_hz=shared.config.frequency_hz,seed=seed,original_initial_fingerprint=digest_array(initial),
            initial_phase_fingerprint=digest_array(phase0),previous_alpha=previous,reference_phase_fingerprint=digest_array(reference),
            maxiter=10000,gtol=shared.config.gtol,source_fingerprint=hashes)
        start=phase0.copy()
    ...
    for job in jobs:
        ...
        row=job['row'];row.update(evidence_status='PRODUCTION',reference_phase_fingerprint=digest_array(reference),reference_seed=seed,
