...
def _display_fields(ctx,output,jobs,cache_only=False):
    ...
    for job in jobs:
        ...
        identity=dict(schema=DISPLAY_SCHEMA,phase=digest_array(phase),positions=digest_array(ctx.positions_m),
            ...
            half_width_m=.007,model_fingerprint=digest_file(Path(p.__file__).with_name("exact_validator.py")))
        ...
        rows.append(dict(initialization=row["initialization"],alpha_per_m=row["alpha_per_m"],
            phase_fingerprint=digest_array(phase),**descriptor))
...
def run(root,ctx=None):
    ...
    for index,alpha in enumerate(ALPHAS):
        ...
        if source_row.initial_phase_fingerprint!=digest_array(original_initial):
    ...
    for alpha in ALPHAS:
        ...
        payload=dict(contract=SCHEMA,method=asdict(method),force_target_n=objective.force_target_n.tolist(),
            ...
            frequency_hz=ctx.config.frequency_hz,seed=seed,original_initial_fingerprint=digest_array(original_initial),
            initial_phase_fingerprint=digest_array(initial),previous_alpha=previous_alpha,
            reference_phase_fingerprint=digest_array(reference_phase),maxiter=10000,gtol=ctx.config.gtol,source_fingerprint=hashes)
    ...
    for job in jobs:
        ...
        job["row"].update(command_cosine_to_fixed_GFE=command_cosine,field_cosine_to_fixed_GFE=field_cosine,
            ...
            reference_phase_fingerprint=digest_array(reference_phase),reference_seed=seed,
    ...
    protocol=dict(schema=SCHEMA,seed=seed,alpha_sequence=[10.,*ALPHAS],nominal_reference_phase_fingerprint=digest_array(reference_phase),
        ...
        reference_frozen_before_sweep=True,paired_origin_initial_fingerprint=digest_array(original_initial),
        force_target_n=nominal_objective.force_target_n.tolist(),maxiter_per_solve=10000,gtol=ctx.config.gtol,
        source_fingerprint=hashes,positions_fingerprint=digest_array(ctx.positions_m),frequency_hz=ctx.config.frequency_hz,
