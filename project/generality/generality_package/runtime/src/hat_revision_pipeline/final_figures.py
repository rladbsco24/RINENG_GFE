...
def _single_vortex_bank(ctx: p.PipelineContext) -> dict[str, Any]:
    ...
    _guard()
    requested = 30 if ctx.config.full else int(os.environ.get("HAT_QUICK_SINGLE_PAIRS", "4"))
...
def figure_1_single_vortex_convergence(ctx: p.PipelineContext) -> mpl.figure.Figure:
    ...
    display_index = (bank["seeds"].index(ctx.memo["production_single_display_seed"])
                     if ctx.memo.get("production_main") else 0)
    ...
    pressure_samples = 181 if ctx.memo.get("production_main") else 161
...
def figure_2_two_branch_evolution(ctx: p.PipelineContext) -> mpl.figure.Figure:
    ...
    for row, component in enumerate(COMPONENTS):
        ...
        for col, checkpoint in enumerate(checkpoints):
            ...
            _draw_target_web_3d(
                ax, conventional, ctx.branch_bank.edge_table,
                color=COLORS["Conventional"], marker="o", line_width=2.3,
                line_alpha=0.20, marker_size=27.0,
            )
            _draw_target_web_3d(
                ax, component_anchors, ctx.branch_bank.edge_table,
...
def _decision_pack(ctx: p.PipelineContext) -> dict[str, Any]:
    ...
    for method in ("Conventional", "FE"):
        ...
        sections[method] = p._native_relative_sections(
            objectives[method], runs[method], directions, line_points=101, plane_points=51
        )
...
def figure_3_local_decision_geometry(ctx: p.PipelineContext) -> mpl.figure.Figure:
    ...
    for column, method in enumerate(("Conventional", "FE")):
        ...
        summary.append(
            {
                "method": method,
                "phase_fingerprint": digest_array(pack["runs"][method].phase_rad),
...
def _tri3_plane(
    ...
    *,
    ...
    points = 101 if ctx.config.full else 61
...
def figure_4_triple_fields_and_decision(ctx: p.PipelineContext) -> mpl.figure.Figure:
    ...
    for method, data in planes.items():
        ...
        local_rows.append(
            {
                "method": method,
                "phase_fingerprint": digest_array(phases[method]),
    ...
    for column, method in enumerate(("Conventional", "FE")):
        ...
        sample_indices = np.rint(np.linspace(5, len(data["q"]) - 6, 7)).astype(int)
...
def _field_concentration(ctx: p.PipelineContext, phase: np.ndarray, target: np.ndarray) -> dict[str, float]:
    half_width = 0.012
    roi_radius = 0.003
    samples = 121 if ctx.config.full else 81
...
def _single_exact_benchmark(ctx: p.PipelineContext) -> dict[str, Any]:
    ...
    viscous_single = raw[["method", "phase_fingerprint", *viscous_columns]].copy()
...
def _figure6_vertical_compensated_fe(
    ...
    payload = {
        ...
        "seed": seed,
        "initial_phase_fingerprint": digest_array(initial_phase),
        ...
        row.update(
            {
                ...
                "target_y_m": float(target[1]),
                "target_z_m": float(target[2]),
                "phase_fingerprint": digest_array(compensated_phase),
                "phase_source": (
...
def _alpha_bank(ctx: p.PipelineContext) -> dict[str, Any]:
    ...
    for index, (alpha, run) in enumerate(zip(ALPHA_VALUES, runs)):
        row = {
            ...
            "phase_fingerprint": digest_array(run.phase_rad), "run_mode": ctx.config.mode,
