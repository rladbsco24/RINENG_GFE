...
from fingerprintlib import fingerprint
import json
import math
import os
...
def _compatible_branch_archive(data_dir: Path, output_root: Path) -> Path:
    ...
    stamp = archive.with_suffix(".fingerprint")
...
def _finite_ka_protocol(ctx: PipelineContext) -> dict[str, Any]:
    ...
    return {
        ...
        "max_nfev_per_start": 70 if ctx.config.full else 14,
        "numerical_root_tolerance": 1.0e-3 if ctx.config.full else 2.5e-3,
        "least_squares": {
        ...
        rows.append(
            {
                ...
                "seed": spec["seed"],
                "phase_fingerprint": digest_array(np.asarray(spec["phase"], dtype=float)),
        ...
        return replace(
            ...
            metadata={
                **dict(run.metadata),
                ...
                "control_group_size": 8,
                "target_count": 3,
                "targets_fingerprint": target_digest,
            },
        )
        ...
        row.update(
            {
                ...
                "phase_fingerprint": phase_digest,
