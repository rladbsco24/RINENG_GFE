...
def _branch_and_local(ctx):
    ...
    for task,method,phase,u,v,fun,source in cases:
        value=fun(phase)
        common=dict(task=task,method=method,phase_fingerprint=digest_array(phase),direction_source=source,
            direction_u_fingerprint=digest_array(np.asarray(u)),direction_v_fingerprint=digest_array(np.asarray(v)),
...
def _mechanics(ctx,items):
    ...
    for item in items:
        ...
        common=dict(task=row["task"],method=row["method"],target_id=row["target_id"],phase_fingerprint=content_id)
    ...
    summary=[]
    for (task,method,content_id),group in mechanics.groupby(["task","method","phase_fingerprint"],sort=False):
        ...
        summary.append(dict(task=task,method=method,phase_fingerprint=content_id,n_targets=len(group),n_resolved=len(resolved),n_distinct_roots=len(distinct),
...
def _quantization(ctx,items):
    ...
    def evaluate(job):
        ...
        return dict(task=row["task"],method=row["method"],target_id=row["target_id"],levels=levels,
            continuous_phase_fingerprint=digest_array(phase),quantized_phase_fingerprint=digest_array(quantized),
...
def _ablations(ctx):
    ...
    for label,config in configurations:
        ...
        for state,phase in (("initial",initial),("terminal",run.phases)):
            ...
            for i,local in enumerate(ev.locals):
                ...
                terms.append(dict(configuration=label,state=state,target_id=local.target_id,
                    ...
                    total_gradient_l2=float(np.linalg.norm(ev.gradient_reduced)),phase_fingerprint=digest_array(np.asarray(phase))))
        for target_index,target in enumerate(benchmark["targets_m"]):
            ...
            row.update(configuration=label,target_id=f"T{target_index+1}",seed=seed,phase_fingerprint=digest_array(np.asarray(run.phases)),
                initial_phase_fingerprint=digest_array(initial),iterations=run.iterations,iteration_cap=10000,
...
def _numerics(ctx,items):
    ...
    for item in representatives:
        ...
        common=dict(task=row["task"],method=row["method"],target_id=row["target_id"],phase_fingerprint=digest_array(phase))
...
def export_results_sentences(table_directory,output_directory,mock_source=None):
    ...
    def add(identifier,table,row_number,message,phase=""):
        ...
        index.append(dict(sentence_id=identifier,source_table=table,source_row_zero_based=row_number,phase_fingerprint=phase,evidence_status="SMOKE"))
    ...
    for i,row in load(name).iterrows():add(f"M3.{i+1:03d}",name,i,f"{row.task} {row.method}, ±0.03 rad: quadratic-fit residual {row.quadratic_relative_residual:.6g}; section Hessian eigenvalues {row.section_hessian_min:.6g}, {row.section_hessian_max:.6g} N/m/rad².",row.phase_fingerprint)
    ...
    for i,row in load(name).iterrows():
        ...
        add(f"M4.{i+1:03d}",name,i,msg,row.phase_fingerprint)
    name="revision_T3_command_summary"
    for i,row in load(name).iterrows():add(f"M4.C{i+1:03d}",name,i,f"{row.task} {row.method}: {row.n_distinct_roots}/{row.n_targets} distinct roots, {row.n_restoring} locally restoring, {row.n_linearly_stable} linearly stable; displacement median/max {row.median_displacement_a:.6g}/{row.maximum_displacement_a:.6g}a; weakest target stiffness {row.weakest_target_stiffness_n_m:.6g} N/m.",row.phase_fingerprint)
    name="revision_T3_surrogate_comparison"
    for i,row in load(name).iterrows():add(f"M5.{i+1:03d}",name,i,f"{row.task} {row.method} {row.target_id}: solid Gor'kov/elastic equilibrium separation {row.equilibrium_difference_um:.6g} µm; same-position Jacobian difference {row.same_position_jacobian_difference_fro_n_m:.6g} N/m.",row.phase_fingerprint)
    name="revision_T4_component_ablations"
    for i,row in load(name).iterrows():add(f"M6.{i+1:03d}",name,i,f"{row.configuration}, {row.target_id}, one smoke seed {row.seed}: displacement {row.finite_ka_displacement_a:.6g}a; weakest stiffness {row.finite_ka_stiffness_min_n_m:.6g} N/m; {row.iterations} iterations.",row.phase_fingerprint)
    name="revision_T6_discrete_phases"
    for i,row in load(name).iterrows():add(f"M8.{i+1:03d}",name,i,f"{row.task} {row.method} {row.target_id}, Q={row.levels}: wrapped RMS phase error {row.phase_rms_error_rad:.6g} rad; equilibrium displacement {row.quantized_displacement_a:.6g}a, equilibrium shift {row.equilibrium_shift_um:.6g} µm; root_found={row.root_found}.",row.quantized_phase_fingerprint)
