...
from hat_revision_pipeline.gorkov_core import Condition, MethodSpec, SingleTargetObjective
...
def _mark_unresolved_lane(ax, table, ylim, unresolved_y):
    """Separate unresolved markers from the quantitative stiffness axis."""
    finite = 1e3 * table.finite_ka_stiffness_min_n_m.to_numpy(float)
    finite = finite[np.isfinite(finite)]
    lowest_value = min(0., float(finite.min())) if len(finite) else 0.
    separator = .5 * (float(unresolved_y) + lowest_value)
    ax.axhspan(ylim[0], separator, facecolor="0.95", edgecolor="none", zorder=0)
    ax.axhline(separator, color="0.7", linewidth=.6, linestyle=":", zorder=1)
    numeric_ticks = [float(value) for value in ax.get_yticks()
                     if separator <= value <= ylim[1]]
    ax.set_yticks([float(unresolved_y), *numeric_ticks])
    tolerance = max(abs(ylim[1] - ylim[0]) * 1e-8, 1e-12)
    ax.yaxis.set_major_formatter(mpl.ticker.FuncFormatter(
        lambda value, position: "Unresolved" if abs(value - unresolved_y) <= tolerance
        else f"{value:.3g}"))
...
def figure_5(ctx):
    ...
    for i, task in enumerate(("Single", "Triple")):
        ...
        for method in reversed(METHOD_ORDER):
            ...
            unresolved_present |= not resolved
            hollow = method in {"FE", "IB", "GS"}
        ...
        if unresolved_present:
            _mark_unresolved_lane(ax, table, ylim, unresolved_y)
        incomplete = table[table.resolved_count.lt(table.endpoint_count)]
        if len(incomplete):
            counts = ", ".join(f"{method} {int(incomplete.loc[method, 'resolved_count'])}/{int(incomplete.loc[method, 'endpoint_count'])}"
                for method in METHOD_ORDER if method in incomplete.index)
            incomplete_notes.append(f"{task} resolved roots: {counts}")
    _legend(fig, y=0.925)
    fig.subplots_adjust(left=0.18 if incomplete_notes else 0.115, right=0.97,
                        top=0.75, bottom=0.20 if incomplete_notes else 0.14)
    if incomplete_notes:
        fig.text(.115, .015, "\n".join(incomplete_notes), ha="left", va="bottom", fontsize=7.5)
...
def figure_6(ctx):
    ...
    context_payload = {"phase": digest_array(phase), "samples": 181 if ctx.memo.get("production_main") else 141 if ctx.config.full else 101,
    ...
    for method, item in selected.items():
        ...
        payload = {"phase": digest_array(local_phase), "target": target.tolist(),
                   "half_width_m": 0.006, "samples": 181 if ctx.memo.get("production_main") else 91,
        ...
        dense[method], _, _ = ctx.cache.get_or_compute(
            ...
            lambda local_phase=local_phase: p._pressure_sections(p._array_field(ctx, local_phase), target,
                     half_xy_m=0.006, half_z_m=0.006, samples=181 if ctx.memo.get("production_main") else 91), recompute=ctx.recompute)
    ...
    for index, method in enumerate(methods, 1):
        ...
        rows.append(dict(item["row"], context_method="FE+g", context_phase_content_id=context_payload["phase"],
...
def alpha_bank(ctx):
    ...
    for alpha in ALPHA_VALUES:
        ...
        payload = {"contract": "feg-main-alpha-transition-v1", "method": asdict(method),
        ...
        run, status, path = ctx.cache.get_or_compute("feg_main_alpha_run", payload,
            lambda objective=objective: solve_corrected_gorkov_fe(objective, initial, maxiter=10000, gtol=ctx.config.gtol),
            recompute=ctx.recompute)
...
def figure_7(ctx):
    bank = alpha_bank(ctx)
    base_samples = 181 if ctx.config.full or ctx.memo.get("production_main") else 161
    # Preserve the reference spatial sampling per wavelength on the broad
    # +/-75 mm field at higher carriers; phase commands remain unchanged.
    samples = int(np.ceil((base_samples - 1) * max(1., ctx.config.frequency_hz / 40000.))) + 1
...
def figure_8(ctx):
    single = original._exact_benchmark_with_concentration(ctx)
    triple = original._triple_exact_benchmark_with_concentration(ctx)
...
def public_captions(ctx):
    ...
    captions = {'Figure_5_single_triple_exact_force_concentration': 'Field concentration C_ROI and the smallest symmetric elastic-bead stiffness eigenvalue for Single and Triple commands. GFE uses the buoyancy-corrected weight as the prescribed upward Gor’kov radiation-force target; FE is the uncompensated control. Each stiffness point is the median over resolved roots with finite stiffness within the task; all methods use the same independent finite-ka elastic-bead validator. The shaded Unresolved lane is categorical and has no stiffness value. Incomplete methods show resolved-root counts over the total number of endpoints.', 'Figure_6_force_field_comparison': 'The broad XY context shows the actual GFE Triple command. The five target-centred XZ cards compare GFE, FE, Conventional, GS and AD at T2 using one shared pressure normalization and independent elastic-bead total-force streamlines. Crosses mark intended targets and open circles mark resolved equilibria. Titles report displacement divided by bead radius and pressure at resolved equilibria. Cards without a resolved root state this explicitly; their open triangles remain labelled unresolved candidates. The exported table includes all three GFE and FE targets.', 'Figure_7_alpha_transition': 'Three GFE commands at alpha = 1e3, 1e6 and 1e7 m^-1, keeping the original common seed, array, target, identity curvature weight, beta = 0 and 10000-iteration L-BFGS-B cap. Each command explicitly includes the effective-weight radiation-force target and has a new phase-verified cache identity. Broad XY pressure sections retain the deposited +/-75 mm domain and individual pressure scales. Each command retains its actual solver status and terminal gradient; a terminal field alone does not establish convergence.', 'Figure_8_performance_time_tradeoff': 'Measured command times are paired with independent elastic-bead equilibrium displacement and equilibrium pressure for Single (top) and Triple (bottom). Triple Conventional, FE and GFE use a shared 30000-iteration ceiling. FE/GFE use compact evaluation with L-BFGS-B for Single and BFGS for Triple; Conventional uses BFGS. Actual terminal iterations and measured timing scopes are retained in the current command tables. IB, GS and AD retain their native iteration protocols. The same pooled broken linear time mapping is used in all four panels. Triple markers are medians across T1-T3 and bars show their minimum-maximum range. GFE is highlighted and uncompensated FE is the control. Cache replay time is not a synthesis benchmark.'}
